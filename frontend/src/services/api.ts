...
export const knowledgeApi = {
  ...
  update: (id: string, data: Record<string,unknown>) => api.patch(`/knowledge/${id}`, data),
...
// Data Ingestion — raw interaction intake (meeting notes, transcripts, emails, CRM updates, notes)
export const ingestionApi = {
  sourceTypes: () => api.get('/ingestion/source-types'),
  list: (params?: Record<string,unknown>) => api.get('/ingestion', { params }),
  get: (id: string) => api.get(`/ingestion/${id}`),
  create: (data: {
    customer_id?: string; source_type: string; title: string; raw_content: string
    auto_process?: boolean; trigger_workflow?: boolean
  }) => api.post('/ingestion', data),
  reprocess: (id: string) => api.post(`/ingestion/${id}/reprocess`),
  triggerWorkflow: (id: string) => api.post(`/ingestion/${id}/trigger-workflow`),
  delete: (id: string) => api.delete(`/ingestion/${id}`),
}
