...
export interface IngestedInteraction {
  id: string; customer_id?: string; source_type: string; title: string;
  raw_content: string; status: 'new' | 'processing' | 'processed' | 'failed';
  detected_sentiment?: string; detected_sentiment_score?: number;
  detected_topics: string[]; detected_risks: string[]; detected_opportunities: string[];
  ai_summary?: string; linked_meeting_id?: string; triggered_workflow: boolean;
  error?: string; created_at?: string; processed_at?: string; customer?: Customer;
}
